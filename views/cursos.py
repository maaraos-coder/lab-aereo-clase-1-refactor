...
def _render_course2_lab1_stage8(lab, saved):
    ...
    # ---------------------------------------------------------
    st.markdown("### 3.2 · De la bomba que ya medimos a la frecuencia que debemos aislar")
    st.write(
        "El Laboratorio B **continúa exactamente con la misma bomba del Laboratorio A**. "
        "Ahora dejamos de diagnosticar y comenzamos a diseñar una solución antivibratoria."
    )
    ...
    with i1:
        _card(
            ...
            f"{rpm:.0f} RPM",
            "RPM significa revoluciones por minuto: cuántas vueltas realiza el eje en un minuto.",
            tone="blue"
        )
    with i2:
        _card(
            "Masa del conjunto",
            f"{mass:.0f} kg",
            "Es la masa de la bomba que utilizaremos para calcular cuánto debe soportar cada apoyo."
        )
    with i3:
        _card(
            "Número de apoyos",
            f"{supports}",
            "En este ejercicio suponemos que la carga se distribuye uniformemente entre cuatro aisladores.",
            tone="green"
        )
    with i4:
        _card(
            "Frecuencia que seguiremos",
            f"{fe:.2f} Hz",
            "Corresponde a una excitación que se repite una vez por cada vuelta del eje.",
            tone="purple"
        )

    st.warning(
        "Los **79 kg** corresponden al conjunto considerado en el caso. "
        "Si en un proyecto real se agrega una bancada de inercia, su masa también debe incluirse antes de calcular las cargas."
    )

    st.markdown("#### A · Primero entendamos qué significa pasar de RPM a Hz")
    st.write(
        "La bomba gira a **2900 RPM**. Eso significa que su eje completa 2900 vueltas en un minuto. "
        "Para analizar vibraciones resulta más práctico expresar esa repetición por segundo. "
        "La unidad que indica cuántas repeticiones ocurren cada segundo es el **hertz (Hz)**."
    )

    st.latex(r"f_{\mathrm{giro}}=\frac{\mathrm{RPM}}{60}")
    st.latex(
        fr"f_{{\mathrm{{giro}}}}=\frac{{{rpm:.0f}}}{{60}}={frot:.2f}\;\mathrm{{Hz}}"
    )
    ...
    with hz1:
        _card(
            "2900 RPM",
            "2900 vueltas por minuto",
            "Es la velocidad de giro indicada para la bomba."
        )
    with hz2:
        _card(
            f"{frot:.2f} Hz",
            f"{frot:.2f} vueltas por segundo",
            "Es la misma velocidad de giro expresada de una forma útil para analizar vibraciones.",
            tone="green"
        )

    st.markdown("#### B · ¿Por qué hablamos de 1×RPM?")
    st.write(
        "Durante cada vuelta pueden existir fuerzas que se repiten periódicamente. "
        "Si una fuerza se repite **una vez por cada revolución**, decimos que aparece en **1×RPM**. "
        "Por eso, para nuestra bomba:"
    )
    st.latex(fr"\boxed{{1\times\mathrm{{RPM}}={fe:.2f}\;\mathrm{{Hz}}}}")

    st.info(
        "Una forma sencilla de imaginarlo: si existiera una pequeña fuerza repetitiva ligada al giro, "
        f"esa fuerza se repetiría aproximadamente **{fe:.2f} veces por segundo**."
    )
    ...
    with fd1:
        _card(
            "1×RPM",
            "una repetición por vuelta",
            f"En esta bomba corresponde a {fe:.2f} Hz. Es la componente que seguiremos en este ejercicio.",
            tone="green"
        )
    with fd2:
        _card(
            "2×RPM",
            "dos repeticiones por vuelta",
            f"Serían aproximadamente {2*fe:.2f} Hz. Puede aparecer en algunos mecanismos, pero no es la frecuencia de diseño adoptada aquí."
        )
    with fd3:
        _card(
            "Paso de álabes",
            "N × frecuencia de giro",
            "Si el impulsor tiene N álabes, el paso sucesivo de esos álabes puede originar otra componente periódica.",
            tone="blue"
        )

    st.write("La relación general para el paso de álabes es:")
    st.latex(r"f_{\mathrm{álabes}}=N\,f_{\mathrm{giro}}")
    st.caption(
        "Que una frecuencia sea posible no significa que necesariamente domine el problema. "
        "En un diagnóstico real se comprueba con mediciones."
    )

    st.markdown("#### C · Entonces, ¿por qué en el Laboratorio A hablábamos de 50 Hz?")
    st.write(
        f"La frecuencia física calculada a partir de las RPM es **{fe:.2f} Hz**. "
        "Pero los espectros del Laboratorio A se mostraron por **bandas de frecuencia**. "
        "La banda que contiene esta componente está representada por su frecuencia central de **50 Hz**."
    )
    st.latex(
        fr"{rpm:.0f}\;\mathrm{{RPM}}"
        fr"\;\longrightarrow\;"
        fr"{fe:.2f}\;\mathrm{{Hz}}"
        r"\;\longrightarrow\;"
        r"\text{banda de }50\;\mathrm{Hz}"
    )
    st.success(
        f"**Para los cálculos del aislador utilizaremos {fe:.2f} Hz, no 50 Hz.** "
        "Los 50 Hz corresponden únicamente a la banda con la que representamos la medición."
    )

    # ---------------------------------------------------------
    # 3.3 CARGA POR APOYO
    # ---------------------------------------------------------
    st.markdown("### 3.3 · ¿Cuánto peso debe soportar cada aislador?")
    st.write(
        "Antes de escoger un resorte necesitamos saber **cuánta carga caerá sobre cada uno**. "
        "La bomba no quedará sostenida por un único aislador: en este ejercicio utilizaremos cuatro apoyos."
    )

    kg_support=mass/supports
    force_support=kg_support*9.81
    lb_support=force_support/4.4482216152605

    st.markdown("#### A · Repartimos la masa entre los cuatro apoyos")
    st.write(
        "Para aprender el procedimiento asumiremos que el centro de gravedad está suficientemente centrado "
        "y que los cuatro apoyos reciben la misma fracción de la masa."
    )
    st.latex(r"m_i=\frac{m_{\mathrm{total}}}{N}")
    st.latex(
        fr"m_i=\frac{{{mass:.0f}}}{{{supports}}}={kg_support:.2f}\;\mathrm{{kg\ por\ apoyo}}"
    )

    st.markdown("#### B · Masa y peso no son exactamente lo mismo")
    st.write(
        "Los **kilogramos (kg)** indican masa. El resorte, en cambio, recibe una **fuerza vertical** debida al peso. "
        "Esa fuerza se expresa en newton (N) y se obtiene multiplicando la masa por la gravedad."
    )
    st.latex(r"W_i=m_i\,g")
    st.latex(
        fr"W_i={kg_support:.2f}\cdot9.81={force_support:.0f}\;\mathrm{{N}}"
    )

    st.markdown("#### C · ¿Por qué después aparece una carga en lb?")
    st.write(
        "El catálogo de resortes que utilizaremos expresa la carga en **lb**. "
        "En este contexto esa cifra representa la carga o fuerza soportada por el aislador. "
        "Por eso convertimos la misma fuerza que acabamos de calcular a las unidades usadas por el catálogo."
    )
    st.latex(
        fr"{force_support:.0f}\;\mathrm{{N}}"
        fr"\div4.448"
        fr"={lb_support:.1f}\;\mathrm{{lb}}"
    )
    st.info(
        f"Los **{kg_support:.2f} kg**, **{force_support:.0f} N** y **{lb_support:.1f} lb** no son tres cargas diferentes. "
        "Son distintas formas de describir la condición estática que debe soportar cada apoyo."
    )

    lc1,lc2,lc3,lc4=st.columns(4)
    with lc1:
        _card("Bomba completa",f"{mass:.0f} kg","Masa total considerada en el ejercicio.",tone="blue")
    with lc2:
        _card("Reparto",f"{supports} apoyos","Hipótesis simplificada: misma carga en cada aislador.")
    with lc3:
        _card("Por apoyo",f"{force_support:.0f} N",f"Equivale a la carga producida por {kg_support:.2f} kg.",tone="green")
    with lc4:
        _card("Dato para catálogo",f"{lb_support:.1f} lb","Este es el valor que compararemos con la capacidad de los resortes.",tone="purple")

    st.warning(
        "En una instalación real la carga no siempre se reparte por igual. "
        "La posición del centro de gravedad, la bancada, el motor, las tuberías y la geometría del montaje "
        "pueden hacer que algunos aisladores reciban más carga que otros."
    )

    # ---------------------------------------------------------
    # 3.4 RIGIDEZ, DEFLEXIÓN Y FRECUENCIA NATURAL
    # ---------------------------------------------------------
    st.markdown("### 3.4 · ¿Por qué un catálogo de resortes habla de carga y deflexión?")
    st.write(
        "Imagina colocar la bomba sobre un resorte. Al apoyar el peso, el resorte **se comprime**. "
        "La cantidad que baja o se acorta por efecto de esa carga se llama **deflexión**."
    )
    ...
    with d1:
        _card(
            "Poca deflexión",
            "el resorte se comprime poco",
            "Para una misma carga, esto suele indicar un resorte más rígido.",
            tone="orange"
        )
    with d2:
        _card(
            "Mayor deflexión",
            "el resorte se comprime más",
            "Para una misma carga, esto suele indicar un resorte más flexible.",
            tone="green"
        )

    st.markdown("#### A · ¿Qué es la rigidez?")
    st.write(
        "La **rigidez** indica cuánto se opone el resorte a deformarse. "
        "Si aplicamos una fuerza grande y el resorte apenas se comprime, es rígido. "
        "Si con esa misma fuerza se comprime más, es más flexible."
    )
    st.write(
        "En un modelo lineal sencillo, la relación entre fuerza, deflexión y rigidez puede escribirse como:"
    )
    st.latex(r"\boxed{k_i\approx\frac{F_i}{\delta_i}}")
    st.caption(
        "k representa rigidez, F la fuerza aplicada y δ la deflexión producida por esa fuerza."
    )

    st.markdown("#### B · ¿Qué ocurre cuando usamos cuatro resortes?")
    st.write(
        "Los cuatro aisladores sostienen simultáneamente la bomba. "
        "Desde el punto de vista vertical actúan como varios resortes trabajando **en paralelo**. "
        "Si son iguales y reciben cargas semejantes, sus rigideces se suman aproximadamente."
    )
    ...
    # ---------------------------------------------------------
    st.markdown("### 3.5 · ¿Cuándo un resorte realmente empieza a aislar?")
    st.write(
        "Ahora tenemos dos frecuencias distintas y necesitamos compararlas:"
    )

    rf1,rf2=st.columns(2)
    with rf1:
        _card(
            "Frecuencia de excitación fₑ",
            f"{fe:.2f} Hz",
            "Proviene del funcionamiento de la bomba. En este ejercicio corresponde a 1×RPM.",
            tone="blue"
        )
    with rf2:
        _card(
            "Frecuencia natural fₙ",
            "depende del aislador",
            "Es la frecuencia propia del conjunto bomba–resortes y cambia según la deflexión del sistema.",
            tone="purple"
        )

    st.write(
        "La comparación se expresa mediante la **razón de frecuencias**:"
    )
    st.latex(r"\boxed{r=\frac{f_e}{f_n}}")

    st.markdown("#### ¿Cómo interpretar r sin pensar todavía en la fórmula?")
    st.write(
        "Si ambas frecuencias son parecidas, el sistema puede entrar en resonancia. "
        "Si la frecuencia de la bomba queda suficientemente por encima de la frecuencia natural, "
    ...
    with reg2:
        _card(
            "r ≈ 1",
            "resonancia",
            "La frecuencia de excitación coincide aproximadamente con la frecuencia natural y la respuesta puede aumentar mucho.",
            tone="orange"
        )
    with reg3:
        _card(
    ...
    st.write(
        "Saber que estamos en la región de aislamiento no basta. También queremos estimar **cuánto** de la fuerza dinámica "
        "de la bomba podría seguir llegando a la losa. Para eso usamos la **transmisibilidad de fuerza**, T_F."
    )
    ...
    with t1:
        _card(
            "T_F = 1",
            "se transmite el 100 %",
            "La amplitud de la fuerza transmitida es igual a la amplitud de la fuerza de excitación."
        )
    with t2:
        _card(
            "T_F = 0,5",
            "se transmite el 50 %",
            "La amplitud de fuerza transmitida se reduce aproximadamente a la mitad."
        )
    with t3:
        _card(
            "T_F = 0,1",
            "se transmite el 10 %",
            "En el modelo idealizado, aproximadamente el 90 % de la amplitud de fuerza considerada deja de transmitirse.",
            tone="green"
        )

    st.write(
        "Por ejemplo, si imaginamos una excitación dinámica de 100 N y obtenemos T_F = 0,1, "
        "el modelo indicaría una fuerza transmitida de aproximadamente 10 N."
    )
    ...
    ax.set_title("Cómo cambia la fuerza transmitida al separar las frecuencias")
    ax.grid(True,alpha=.22)
    fig.tight_layout()
    st.pyplot(fig,use_container_width=True)
    plt.close(fig)

    st.markdown("**Cómo leer la gráfica**")
    st.markdown(
        """
    ...
    # ---------------------------------------------------------
    # 3.6 CRITERIO DE DISEÑO ADOPTADO PARA EL EJERCICIO
    # ---------------------------------------------------------
    st.markdown("### 3.6 · ¿Qué criterio de diseño adoptaremos en este ejercicio?")
    st.write(
        "En un proyecto real, la selección de un aislador no se obtiene de un porcentaje elegido al azar. "
        "El criterio depende del equipo, de la estructura, del recinto receptor, de los requisitos del proyecto "
        "y de las referencias técnicas que utilice el especialista."
    )
    st.write(
        "Para que podamos practicar todo el procedimiento con un caso concreto, en este laboratorio adoptaremos "
        "un **criterio de deflexión estática mínima** para el sistema antivibratorio."
    )

    design_delta_mm=19.0
    delta_min_mm=design_delta_mm
    fn_max=fn_delta(delta_min_mm)
    r_req=fe/max(fn_max,1e-12)
    tf_target=tf_force(r_req,zeta)
    target_iso=max(0.0,(1.0-tf_target)*100.0)

    st.markdown("#### Criterio adoptado para nuestro caso")
    st.latex(fr"\boxed{{\delta_{{\min}}={delta_min_mm:.0f}\;\mathrm{{mm}}}}")
    st.info(
        "Los **19 mm** son el criterio de diseño adoptado para este ejercicio didáctico. "
        "No significan que todas las bombas deban instalarse siempre con 19 mm de deflexión."
    )

    st.markdown("#### ¿Qué significa físicamente pedir al menos 19 mm?")
    st.write(
        "Significa que, cuando la bomba esté apoyada sobre los resortes y éstos reciban su carga real de operación, "
        "queremos que el sistema pueda comprimirse **al menos 19 mm**. "
        "Esa deflexión fija una frecuencia natural máxima para el montaje."
    )

    st.latex(
        fr"f_{{n,\max}}="
        fr"\frac{{1}}{{2\pi}}\sqrt{{\frac{{9.81}}{{{delta_min_mm/1000:.3f}}}}}"
        fr"={fn_max:.2f}\;\mathrm{{Hz}}"
    )

    st.write(
        f"Ahora comparamos esa frecuencia natural con los **{fe:.2f} Hz** de excitación de la bomba:"
    )
    st.latex(
        fr"r=\frac{{f_e}}{{f_n}}"
        fr"=\frac{{{fe:.2f}}}{{{fn_max:.2f}}}"
        fr"={r_req:.2f}"
    )

    st.write(
        "Finalmente, con la razón de frecuencias y el amortiguamiento seleccionado en la sección anterior, "
        "el modelo entrega la transmisibilidad estimada:"
    )
    st.latex(fr"T_F\approx{tf_target:.3f}")

    cr1,cr2,cr3,cr4=st.columns(4)
    with cr1:
        _card(
            "Criterio del ejercicio",
            f"δ ≥ {delta_min_mm:.0f} mm",
            "Es el punto de partida adoptado para seleccionar el resorte.",
            tone="blue"
        )
    with cr2:
        _card(
            "Frecuencia natural",
            f"fₙ ≤ {fn_max:.2f} Hz",
            "Es la frecuencia natural máxima asociada a una deflexión de 19 mm."
        )
    with cr3:
        _card(
            "Separación dinámica",
            f"r ≥ {r_req:.2f}",
            f"Compara los {fe:.2f} Hz de la bomba con la frecuencia natural del montaje.",
            tone="purple"
        )
    with cr4:
        _card(
            ...
            f"T_F ≈ {tf_target:.3f}",
            f"Equivale aproximadamente a {target_iso:.1f} % de reducción de la amplitud de fuerza en este modelo.",
            tone="green"
        )

    st.success(
        f"Observa el orden del razonamiento: **adoptamos 19 mm → obtenemos fₙ ≈ {fn_max:.2f} Hz → "
        f"calculamos r ≈ {r_req:.2f} → estimamos T_F ≈ {tf_target:.3f}**. "
        "El porcentaje de aislamiento aparece como resultado del modelo, no como un número arbitrario elegido al comienzo."
    )

    # ---------------------------------------------------------
    # 3.7 PRESELECCIÓN TÉCNICA DE FAMILIAS DE RESORTE
    # ---------------------------------------------------------
    st.markdown("### 3.7 · ¿Cómo pasamos del cálculo a un resorte real de catálogo?")
    st.write(
        "Ya sabemos dos cosas fundamentales: cada aislador deberá soportar aproximadamente "
        f"**{lb_support:.1f} lb** y queremos que, bajo esa carga, el sistema desarrolle al menos **{delta_min_mm:.0f} mm de deflexión**."
    )

    st.markdown("#### A · Primero entendamos la unidad “pulgada”")
    st.write(
        "Los catálogos norteamericanos suelen expresar la deflexión de los resortes en **in**, abreviatura de *inch* o pulgada. "
        "Para evitar confusiones mostraremos siempre su equivalente en milímetros."
    )
    ...
    with conv1:
        _card(
            "Familia de 1 pulgada",
            "1 in = 25,4 mm",
            "Su denominación indica una deflexión nominal cercana a una pulgada cuando el resorte trabaja alrededor de su carga nominal.",
            tone="blue"
        )
    with conv2:
        _card(
            "Familia de 4 pulgadas",
            "4 in = 101,6 mm",
            "Su denominación indica una deflexión nominal cercana a cuatro pulgadas cuando trabaja alrededor de su carga nominal.",
            tone="purple"
        )

    st.markdown("#### B · “Deflexión nominal” no significa “deflexión que siempre tendrá”")
    st.write(
        "Este es uno de los puntos más importantes del catálogo. "
        "El fabricante declara una **carga nominal** y la **deflexión que el resorte desarrolla a esa carga**."
    )

    nom1,nom2=st.columns(2)
    with nom1:
        _card(
            "Ejemplo de etiqueta",
            "50 lb → 0,97 in",
            "Significa: cuando ese resorte soporta aproximadamente 50 lb, se comprime alrededor de 0,97 pulgadas."
        )
    with nom2:
        _card(
            "Nuestra bomba",
            f"{lb_support:.1f} lb por apoyo",
            "Como nuestra carga no es exactamente 50 lb, la compresión real tampoco tiene por qué ser exactamente 0,97 pulgadas.",
            tone="green"
        )
    ...
    st.markdown("#### C · Ejemplo 1: un resorte de la familia de 1 pulgada")
    st.write(
        "Tomemos como ejemplo preliminar el **FDS 1-50**. "
        "Su ficha indica aproximadamente **50 lb de carga nominal** y **0,97 in de deflexión nominal**."
    )
    st.write(
        f"Nuestra bomba aplica **{lb_support:.1f} lb**, es decir, algo menos que las 50 lb nominales. "
        "Por eso esperamos que el resorte se comprima algo menos que 0,97 in."
    )
    st.latex(
        fr"\delta_{{\mathrm{{op}}}}\approx"
        fr"0.97\;\mathrm{{in}}\;"
    ...
    with e12:
        _card("Deflexión estimada",f"{one_op_mm:.1f} mm",f"Supera el criterio de {delta_min_mm:.0f} mm.",tone="green")
    with e13:
    ...
    if one_op_mm >= delta_min_mm:
        st.success(
            f"En esta aproximación, el FDS 1-50 desarrolla **{one_op_mm:.1f} mm**, "
            f"por lo que supera el criterio adoptado de **{delta_min_mm:.0f} mm**. "
            "Por eso es un candidato razonable para comprobar después con el catálogo."
        )
    else:
        st.warning(
            "En esta aproximación, el modelo de 1 pulgada no alcanzaría la deflexión requerida."
        )

    st.markdown("#### D · Ejemplo 2: ¿por qué una familia de 4 pulgadas no se comprime automáticamente 4 pulgadas?")
    st.write(
        "Ahora observemos un **FDS 4-100**. Su denominación de 4 pulgadas corresponde a su comportamiento nominal "
        "cerca de **100 lb**. Nuestra bomba solo aplica aproximadamente "
        f"**{lb_support:.1f} lb**, bastante menos que esa carga."
    )
    st.write(
        "Por eso sería incorrecto asumir que, apenas instalemos nuestra bomba, el resorte se comprimirá 101,6 mm."
    )
    st.latex(
        fr"\delta_{{\mathrm{{op}}}}\approx"
        fr"4.00\;\mathrm{{in}}\;"
    ...
    with e22:
        _card("Nuestra carga",f"{lb_support:.1f} lb","El resorte estaría trabajando bastante por debajo de su carga nominal.",tone="orange")
    with e23:
    ...
    st.markdown("#### F · ¿Qué buscaremos finalmente en el catálogo?")
    next1,next2,next3,next4=st.columns(4)
    with next1:
        _card(
            "1 · Capacidad de carga",
            f"≥ {lb_support:.1f} lb",
            "El aislador debe soportar la carga real que recibe cada apoyo.",
            tone="blue"
        )
    with next2:
        _card(
            "2 · Deflexión de operación",
            f"≥ {delta_min_mm:.0f} mm",
            "No basta con mirar la deflexión nominal; debemos estimar la que realmente desarrolla con nuestra carga.",
            tone="green"
        )
    with next3:
        _card(
            "3 · Frecuencia natural",
            f"≤ {fn_max:.2f} Hz",
            "Se obtiene a partir de la deflexión real del aislador."
        )
    with next4:
        _card(
            "4 · Transmisibilidad",
            f"≈ {tf_target:.3f} o menor",
            "Sirve como verificación dinámica del comportamiento idealizado.",
            tone="purple"
        )

    st.success(
        "La idea central es: **primero conocemos la carga real de la bomba, luego exigimos una deflexión de operación y recién después buscamos el modelo comercial**. "
        "No escogemos un resorte solo porque tenga más pulgadas o una capacidad de carga más grande."
    )
    ...
    st.write(
        "Ya no iremos al catálogo a probar modelos al azar. Llevamos una ficha de selección construida a partir del caso."
    )
    ...
    current_design={
        "saved":False,
        "design_version":"deflection_19mm_v1",
        "design_basis":"Criterio didáctico de deflexión estática mínima de 19 mm",
        "rpm":float(CASE_RPM),
    ...
    if isinstance(saved_design,dict) and saved_design.get("saved"):
        st.success(
            f"Especificación activa: {saved_design['load_per_support_lb']:.1f} lb/apoyo · "
            f"δ ≥ {saved_design['min_deflection_mm']:.0f} mm · "
            f"fₙ ≤ {saved_design['max_fn_hz']:.2f} Hz · "
            f"T_F de referencia ≈ {saved_design['target_tf']:.3f}."
        )
...
_C2L1_STAGE9_CLASS_ID = "clase-03-impacto-instalaciones-lab-1"

_C2L1_STAGE9_QUESTIONS = [
    # ------------------------------------------------------------------
    # ETAPA 1 · Del fenómeno a la cadena vibroacústica
    # ------------------------------------------------------------------
    {
        "stage":1,
        "title":"Cadena vibroacústica",
        "question":"Una fuerza dinámica actúa sobre un elemento del edificio. ¿Cuál representa mejor la secuencia física estudiada?",
        "options":[
            "Fuerza → vibración de la estructura → propagación mecánica → radiación de sonido.",
        ...
        "correct":0,
        "explanation":"Una excitación mecánica puede hacer vibrar la estructura; esa vibración se propaga por elementos sólidos y determinadas superficies pueden luego radiar sonido."
    },
    {
        "stage":1,
        "title":"Fuente, mecanismo y camino",
        "question":"Al diagnosticar ruido producido por una bomba, ¿por qué no basta con identificar solamente la bomba como fuente?",
        "options":[
            "Porque toda bomba produce exactamente el mismo ruido.",
            "Porque también debemos identificar qué mecanismo genera la excitación y por qué caminos llega la energía al receptor.",
            "Porque la fuente nunca participa en el problema.",
            "Porque solo interesa medir el nivel acústico del dormitorio."
        ],
        "correct":1,
        "explanation":"El enfoque del laboratorio distingue fuente, mecanismo y caminos de transmisión. Una misma bomba puede transmitir energía por apoyos, tuberías y radiación aérea."
    },

    # ------------------------------------------------------------------
    # ETAPA 2 · Respuesta dinámica de la estructura
    # ------------------------------------------------------------------
    {
        "stage":2,
        "title":"Movilidad mecánica",
        "question":"Dos estructuras reciben la misma fuerza dinámica F(f). Si la estructura A posee mayor movilidad Y(f), ¿qué esperamos para su velocidad vibratoria v(f)?",
        "options":[
            "Será menor, porque una mayor movilidad frena la vibración.",
            "Será mayor, porque v(f) = Y(f) · F(f).",
            "Será necesariamente cero.",
            "No puede relacionarse con la movilidad."
        ],
        "correct":1,
        "explanation":"Para una misma fuerza, una mayor movilidad implica una mayor velocidad vibratoria. La movilidad es una propiedad dependiente de la estructura y de la frecuencia."
    },
    {
        "stage":2,
        "title":"Frecuencia natural",
        "question":"En el modelo masa–resorte ideal, ¿qué ocurre generalmente con la frecuencia natural si aumenta la rigidez k y la masa permanece constante?",
        "options":[
            "Disminuye.",
            "Aumenta.",
        ...
        "correct":1,
        "explanation":"En f₀ = (1/2π)√(k/m), aumentar la rigidez manteniendo la masa eleva la frecuencia natural."
    },
    {
        "stage":2,
        "title":"Vibración y radiación",
        "question":"Una superficie presenta una velocidad vibratoria elevada. ¿Podemos concluir solamente con ese dato que radiará mucho sonido?",
        "options":[
            "Sí, siempre.",
            "No. También intervienen el área, la frecuencia y la eficiencia de radiación, entre otros factores.",
            "Sí, pero únicamente bajo 100 Hz.",
            "No, porque una superficie vibrante nunca puede radiar sonido."
        ],
        "correct":1,
        "explanation":"Vibrar mucho no equivale automáticamente a radiar mucho sonido. La potencia radiada depende también de la superficie y de su eficiencia de radiación."
    },

    # ------------------------------------------------------------------
    # ETAPA 3 · Diagnóstico mediante mediciones y caminos
    # ------------------------------------------------------------------
    {
        "stage":3,
        "title":"Diseño de campaña",
        "question":"Antes de decidir una medida de control, ¿qué estrategia corresponde al enfoque de diagnóstico de la Etapa 3?",
        "options":[
            "Medir únicamente en el receptor y comprar la solución con mayor aislamiento.",
            "Formular hipótesis, medir fuente y caminos candidatos, comparar resultados y realizar una prueba de confirmación.",
            "Instalar primero el tratamiento y medir solamente después.",
            "Elegir el camino de transmisión solo por inspección visual."
        ],
        "correct":1,
        "explanation":"La Etapa 3 construye el diagnóstico mediante hipótesis, campaña de medición, comparación de caminos y pruebas de confirmación."
    },
    {
        "stage":3,
        "title":"Comparación de caminos",
        "question":"Si dos caminos reciben una fuerza dinámica comparable y uno presenta mayor movilidad en la frecuencia investigada, ¿qué indica el modelo v = Y·F?",
        "options":[
            "Ese camino tenderá a desarrollar mayor velocidad vibratoria.",
        ...
        "correct":0,
        "explanation":"Con fuerzas comparables, el camino de mayor movilidad desarrolla mayor velocidad vibratoria y puede constituir un camino de transmisión relevante."
    },
    {
        "stage":3,
        "title":"Prueba de confirmación",
        "question":"¿Para qué sirve modificar temporalmente un camino sospechoso y observar qué ocurre en el receptor?",
        "options":[
            "Para demostrar que cualquier cambio de nivel es una coincidencia.",
        ...
        "correct":1,
        "explanation":"Una prueba de confirmación fortalece o debilita la hipótesis causal sobre un camino de transmisión."
    },

    # ------------------------------------------------------------------
    # ETAPA 4 · Del impacto a la respuesta del piso
    # ------------------------------------------------------------------
    {
        "stage":4,
        ...
        "options":[
            "Porque el impacto contiene energía distribuida en distintas frecuencias y queremos estudiar cómo responde el piso en cada una.",
        ...
        "correct":0,
        "explanation":"El impacto es transitorio y contiene contenido frecuencial. La representación F(f) permite relacionar la excitación con la respuesta dinámica del piso por frecuencia."
    },
    {
        "stage":4,
        "title":"Mismo impacto, distinto piso",
        "question":"Dos pisos reciben la misma excitación F(f), pero poseen movilidades diferentes. ¿Qué explica que sus respuestas vibratorias sean distintas?",
        "options":[
            "La relación v(f) = Y(f) · F(f).",
        ...
        "correct":0,
        "explanation":"Con la misma excitación, diferencias en Y(f) producen diferencias en v(f). Esa es la base de la comparación realizada en la Etapa 4."
    },
    {
        "stage":4,
        "title":"De vibración a sonido",
        "question":"Después de calcular la velocidad vibratoria v(f), ¿qué idea adicional necesitamos antes de interpretar el sonido radiado?",
        "options":[
            "La capacidad de la superficie vibrante para radiar acústicamente.",
            "Solo la temperatura interior del edificio.",
            "La cantidad de preguntas del laboratorio.",
            "Ninguna: v(f) es directamente Lₙ(f)."
        ],
        "correct":0,
        "explanation":"La cadena continúa desde v(f) hacia potencia radiada y finalmente nivel de ruido de impacto. La respuesta vibratoria no es directamente un nivel acústico."
    },

    # ------------------------------------------------------------------
    # ETAPA 5 · Predicción de la losa base
    # ------------------------------------------------------------------
    {
        "stage":5,
        ...
        "correct":1,
        "explanation":"Lₙ,₀(f) es la referencia espectral de la losa sin el tratamiento que posteriormente se diseñará."
    },
    {
        "stage":5,
        ...
        "options":[
            "Separa dos regímenes de comportamiento/radiación que utilizan expresiones de cálculo diferentes.",
        ...
        "correct":0,
        "explanation":"La posición de cada banda respecto de f_c determina el régimen utilizado por el modelo y está vinculada al acoplamiento de las ondas de flexión con el aire."
    },
    {
        "stage":5,
        "title":"Predicción por bandas",
        "question":"¿Por qué la Etapa 5 construye Lₙ,₀(f) banda por banda en vez de entregar directamente un único valor?",
        "options":[
            "Porque el comportamiento de la losa cambia con la frecuencia y el modelo es espectral.",
        ...
        "correct":0,
        "explanation":"La respuesta y la radiación de la losa dependen de la frecuencia; por eso se construye una curva espectral."
    },

    # ------------------------------------------------------------------
    # ETAPA 6 · Piso flotante y mejora ΔLₙ(f)
    # ------------------------------------------------------------------
    {
        "stage":6,
        ...
        "options":[
            "Una masa superior desacoplada de la losa base mediante un elemento resiliente.",
        ...
        "correct":0,
        "explanation":"El piso flotante introduce una masa desacoplada mediante una capa o apoyos resilientes, formando un sistema dinámico masa–resorte."
    },
    {
        "stage":6,
        "title":"Rigidez dinámica y f₀",
        "question":"Manteniendo las masas, si el elemento resiliente es más flexible y disminuye su rigidez dinámica superficial s′, ¿qué tendencia esperamos para f₀?",
        "options":[
            "f₀ tiende a disminuir.",
            "f₀ tiende a aumentar.",
            "f₀ se hace necesariamente igual a f_c.",
            "La rigidez no influye en f₀."
        ],
        "correct":0,
        "explanation":"En el modelo estudiado una menor rigidez dinámica, manteniendo las masas, reduce la frecuencia natural del sistema."
    },
    {
        "stage":6,
        "title":"Mejora ΔLₙ(f)",
        ...
        "options":[
            "El nivel final absoluto del piso terminado.",
        ...
        "correct":1,
        "explanation":"ΔLₙ(f) es una diferencia de niveles que expresa la mejora del tratamiento por banda. Todavía no es Lₙ,final(f)."
    },
    {
        "stage":6,
        "title":"Puentes rígidos",
        "question":"¿Qué efecto puede tener un contacto rígido accidental que puentea el elemento resiliente de un piso flotante?",
        "options":[
            "Puede crear un camino mecánico paralelo, aumentar la rigidez equivalente y degradar el desacoplamiento.",
            "Siempre mejora el aislamiento.",
            "Solo modifica el color del acabado.",
            "No tiene ningún efecto en la transmisión."
        ],
        "correct":0,
        "explanation":"Los puentes rígidos alteran el sistema ideal y pueden degradar fuertemente el desempeño previsto del piso flotante."
    },

    # ------------------------------------------------------------------
    # ETAPA 7 · Curva final y selección profesional
    # ------------------------------------------------------------------
    {
        "stage":7,
        "title":"Curva final",
        "question":"Si ya conocemos Lₙ,₀(f) y la mejora ΔLₙ(f), ¿cómo construimos la predicción del piso terminado?",
        "options":[
        ...
        "correct":0,
        "explanation":"Como ΔLₙ(f) está definida como mejora en dB respecto de la condición base, se resta banda por banda de Lₙ,₀(f)."
    },
    {
        "stage":7,
        "title":"Selección profesional",
        "question":"Al seleccionar un sistema real desde catálogo, ¿por qué no basta con escoger el producto con mayor ΔLw declarado?",
        "options":[
            "Porque la decisión también debe considerar el desempeño espectral, masa añadida, carga admisible, espesor y restricciones constructivas.",
        ...
        "correct":0,
        "explanation":"La Etapa 7 combina desempeño acústico y viabilidad constructiva. Un índice ponderado no sustituye la evaluación espectral ni las restricciones del proyecto."
    },

    # ------------------------------------------------------------------
    # ETAPA 8 · Bomba, NPSH y aisladores
    # ------------------------------------------------------------------
    {
        "stage":8,
        "title":"NPSH y cavitación",
        "question":"En el caso de la bomba, NPSH_A ≈ 3,41 m y NPSH_R ≈ 3,8 m. ¿Cuál es la interpretación más correcta?",
        "options":[
            "Existe un margen hidráulico negativo y una condición desfavorable compatible con riesgo de cavitación; por sí sola no demuestra acústicamente que exista cavitación.",
        ...
        "correct":0,
        "explanation":"NPSH_A pertenece a la instalación y NPSH_R a la bomba/catálogo. Un margen negativo es evidencia hidráulica desfavorable, pero el diagnóstico de cavitación se fortalece con evidencia vibroacústica y operacional."
    },
    {
        "stage":8,
        ...
        "options":[
            "2900 RPM → 48,33 Hz de frecuencia física → se representa principalmente en la banda de 50 Hz.",
            "2900 RPM → 2900 Hz → banda de 3000 Hz.",
            "2900 RPM → 50 Hz exactos de giro.",
            "2900 RPM no puede convertirse a hertz."
        ],
        "correct":0,
        "explanation":"La frecuencia de giro es 2900/60 ≈ 48,33 Hz. En el espectro por bandas del laboratorio esa componente aparece representada principalmente en la banda central de 50 Hz."
    },
    {
        "stage":8,
        "title":"Carga por aislador",
        "question":"La bomba considerada tiene una masa de 79 kg y se reparte uniformemente en 4 apoyos. ¿Cuál es aproximadamente la carga que llevamos al catálogo por aislador?",
        "options":[
            "19,75 kg de masa por apoyo, equivalentes a unos 194 N o 43,6 lb de carga.",
            "79 kg y 194 lb por apoyo.",
            "4 kg por apoyo.",
            "2900 lb por apoyo."
        ],
        "correct":0,
        "explanation":"79/4 = 19,75 kg por apoyo. El peso correspondiente es aproximadamente 194 N, equivalente a unos 43,6 lbf."
    },
    {
        "stage":8,
        "title":"Deflexión y frecuencia natural",
        "question":"Para el ejercicio se adopta una deflexión estática mínima de 19 mm. ¿Qué significa aproximadamente ese criterio?",
        "options":[
            "19 mm ≈ 0,75 in y corresponde a una frecuencia natural de aproximadamente 3,62 Hz en el modelo vertical idealizado.",
            "19 mm = 19 in y corresponde a 48,33 Hz.",
            "19 mm es la carga nominal del resorte.",
            "19 mm es un nivel acústico."
        ],
        "correct":0,
        "explanation":"19 mm = 0,019 m ≈ 0,75 in. Aplicando f_n = (1/2π)√(g/δ), se obtiene aproximadamente 3,62 Hz."
    },
    {
        "stage":8,
        "title":"Catálogo y deflexión de operación",
        "question":"Un FDS 1-50 declara 50 lb y 0,97 in nominales. Si nuestra carga real es aproximadamente 43,6 lb, ¿qué debemos comparar con el criterio de 19 mm?",
        "options":[
            "La deflexión de operación estimada: aproximadamente 0,85 in ≈ 21,5 mm, que supera el criterio de 19 mm.",
        ...
        "correct":0,
        "explanation":"La denominación nominal no basta. Con la aproximación lineal δ_op ≈ δ_nom(F_op/F_nom), la carga real entrega alrededor de 0,85 in ≈ 21,5 mm de deflexión de operación."
    },
]
...
def _c2l1_stage9_submission():
    """Devuelve solo una entrega FINAL compatible con la versión actual."""
    user_key=st.session_state.get("user_key")
    if not user_key:
        return None
    rows=_remote_rows(
    ...
    row=next(
        (
            r for r in rows
            if int(r.get("stage") or -1)==9
            and r.get("question_key")=="final_comprehension"
        ),
        None
    )
    if not row:
    ...
    if not isinstance(payload,dict):
        payload={}
    # Las entregas de la Etapa 9 antigua no cierran la nueva evaluación.
    if payload.get("version") != _C2L1_STAGE9_VERSION:
        return None
    return {"row":row,"payload":payload}


def _c2l1_stage9_reset_old_local_state(saved):
    """Migra una sola vez desde la Etapa 9 antigua a la versión basada en Etapas 1–8."""
    if saved.get("stage9_content_version") == _C2L1_STAGE9_VERSION:
        return
    for key in list(st.session_state.keys()):
        if (
            key.startswith("e9_q")
            or key.startswith("e9_checked_")
            or key in {
                "e9_submitted",
                "e9_score",
                "e9_saved_answers",
                "e9_confirm_incomplete",
            }
        ):
            st.session_state.pop(key,None)
    saved.pop("stage9_draft",None)
    saved["stage9_content_version"]=_C2L1_STAGE9_VERSION
    saved["stage9_updated_at"]=_now()
    _save_future_state_impl(_C2L1_STAGE9_CLASS_ID,saved)


def _c2l1_stage9_restore_draft(saved):
    """Restaura respuestas y comprobaciones guardadas para que el alumno pueda continuar otro día."""
    draft=saved.get("stage9_draft")
    if not isinstance(draft,dict):
        return
    if draft.get("version") != _C2L1_STAGE9_VERSION:
        return
...
def _render_course2_lab1_stage9(lab, saved):
    ...
    # La antigua fila INTERPRETARÁS / COMPROBARÁS / DECIDIRÁS fue eliminada:
    # pertenecía a una versión previa del laboratorio.
    st.write(
        "Esta evaluación revisa exclusivamente los contenidos desarrollados en las **Etapas 1 a 8 de este laboratorio**. "
        "Las preguntas recorren desde la cadena vibroacústica y la respuesta estructural hasta la predicción de ruido de impacto, "
    ...
    if st.session_state.get("role")=="Docente":
        _c2l1_stage9_teacher_view()
        return

    # Migrar cualquier estado local de la Etapa 9 antigua y restaurar el borrador.
    _c2l1_stage9_reset_old_local_state(saved)
    remote=_c2l1_stage9_submission()
    submitted=bool(remote or st.session_state.get("e9_submitted"))

    # ------------------------------------------------------------------
    # EVALUACIÓN YA ENVIADA: siempre se puede volver a visualizar.
    # ------------------------------------------------------------------
    if submitted:
        ...
        score=float(
            ...
            if row and row.get("teacher_score") is not None
                ...
                row.get("auto_score")
                if row
                else st.session_state.get("e9_score",0)
            ) or 0
        )
        correct=sum(
        ...
        st.caption(
            ...
            "tus respuestas, la pauta y la retroalimentación."
        )
        ...
        for i,item in enumerate(_C2L1_STAGE9_QUESTIONS):
            ...
            with st.container(border=True):
                ...
                if chosen==correct_opt:
                    st.success("✓ CORRECTO · 4/4 puntos")
                else:
                    st.error("✗ INCORRECTO · 0/4 puntos")
                    st.write(f"**Respuesta correcta:** {correct_opt}")
                st.info(item["explanation"])

        st.markdown("### Siguiente paso")
        st.write(
            "La Etapa 10 integra estos contenidos en un problema donde deberás diagnosticar, calcular, "
            "seleccionar y justificar una solución técnica."
        )
        return

    # ------------------------------------------------------------------
    # BORRADOR EN CURSO: restaurar estado persistente por alumno.
    # ------------------------------------------------------------------
    _c2l1_stage9_restore_draft(saved)
    ...
    if st.button(
        ...
        answered,checked=_c2l1_stage9_save_draft(saved)
        st.success(
            f"Avance guardado: {answered}/25 respuestas seleccionadas · "
        ...
        if not checked:
            if st.button(
                ...
                if choice is None:
                    ...
                    st.session_state[f"e9_checked_{i}"]=True
                    # Cada comprobación se persiste inmediatamente por alumno.
                    _c2l1_stage9_save_draft(saved)
                    st.rerun()
        else:
            correct_opt=item["options"][item["correct"]]
            chosen=st.session_state.get(f"e9_q{i}")
            if chosen==correct_opt:
                st.success("✓ CORRECTO · 4/4 puntos")
            else:
                st.error("✗ INCORRECTO · 0/4 puntos")
    ...
    answered=sum(
        st.session_state.get(f"e9_q{i}") is not None
        for i in range(25)
    )
    checked=sum(
        bool(st.session_state.get(f"e9_checked_{i}"))
        for i in range(25)
    )
    provisional=sum(
        4 for i,item in enumerate(_C2L1_STAGE9_QUESTIONS)
        if st.session_state.get(f"e9_checked_{i}")
        and st.session_state.get(f"e9_q{i}")==item["options"][item["correct"]]
    )

    st.markdown("---")
    st.progress(checked/25)
    p1,p2,p3=st.columns(3)
    p1.metric("Seleccionadas",f"{answered}/25")
    p2.metric("Comprobadas",f"{checked}/25")
    p3.metric("Puntos acumulados",f"{provisional}/100")

    st.caption(
        "El puntaje definitivo se registra al enviar la evaluación. "
        "Cada pregunta vale 4 puntos y el máximo total es 100 puntos."
    )
    ...
    if st.button(
        ...
        answered_s,checked_s=_c2l1_stage9_save_draft(saved)
        st.success(
            f"Avance guardado: {answered_s}/25 respuestas seleccionadas · "
            f"{checked_s}/25 preguntas comprobadas. Puedes salir y volver más tarde."
        )
    ...
    if st.button(
        ...
        type="primary",
        use_container_width=True,
        key="e9_submit_button"
    ):
        # Guardar primero, incluso antes de validar si está completa.
        _c2l1_stage9_save_draft(saved)
        if checked<25:
            st.warning(
                f"Aún faltan {25-checked} preguntas por comprobar. "
                "Tu avance ya quedó guardado."
            )
            st.session_state["e9_confirm_incomplete"]=True
        else:
            _c2l1_finish_stage9(saved,"submitted")
            st.rerun()

    if st.session_state.get("e9_confirm_incomplete") and checked<25:
        st.warning(
            "Si envías ahora, las preguntas pendientes quedarán sin puntaje. "
            "Puedes volver a completarlas más tarde mientras no confirmes el envío."
        )
        if st.button(
            "Confirmar envío con respuestas pendientes",
            key="e9_submit_incomplete",
            use_container_width=True
        ):
            _c2l1_finish_stage9(saved,"submitted_incomplete")
            st.rerun()
...
def _c2l2_stage1(lab,saved):
    ...
    _c2l2_stage_header(
        1,
        "DE LA PREDICCIÓN AL DESCRIPTOR PONDERADO",
        "Distingue la curva predicha del Laboratorio 1 de una medición normalizada y reconoce qué magnitud se pondera en laboratorio o en terreno."
    )

    # ------------------------------------------------------------------
    # 1 · CONTINUIDAD CON EL LABORATORIO 1
    # ------------------------------------------------------------------
    st.markdown(
        """
    ...
    st.markdown("## 1 · ¿Qué traemos realmente del Laboratorio 1?")
    st.write(
        "En el Laboratorio 1 **no realizamos un ensayo normalizado ni una medición in situ**. "
        "Construimos una **predicción espectral** del piso terminado, combinando la predicción de la losa base "
        "con la mejora estimada del sistema de piso flotante."
    )
    st.latex(r"\boxed{L_{n,\mathrm{final}}(f)=L_{n,0}(f)-\Delta L_n(f)}")

    st.markdown(
        """
        <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin:.7rem 0 1rem">
          <div style="border:1px solid #bfdbfe;border-radius:17px;padding:16px;background:#eff6ff;min-height:174px">
            <div style="font-size:.72rem;font-weight:850;letter-spacing:.07em;color:#075985">LABORATORIO 1</div>
            <div style="font-size:1.25rem;font-weight:850;color:#0f172a;margin:.4rem 0">Lₙ,final(f)</div>
            <div style="color:#526174;line-height:1.48">Curva <b>predicha</b> banda por banda mediante modelos físicos y acústicos.</div>
          </div>
          <div style="border:1px solid #a7f3d0;border-radius:17px;padding:16px;background:#ecfdf5;min-height:174px">
            <div style="font-size:.72rem;font-weight:850;letter-spacing:.07em;color:#047857">LABORATORIO 2</div>
            <div style="font-size:1.25rem;font-weight:850;color:#0f172a;margin:.4rem 0">ISO 717-2</div>
            <div style="color:#526174;line-height:1.48">Usaremos la curva predicha como <b>entrada didáctica</b> para aprender el procedimiento de ponderación.</div>
          </div>
          <div style="border:1px solid #ddd6fe;border-radius:17px;padding:16px;background:#f5f3ff;min-height:174px">
            <div style="font-size:.72rem;font-weight:850;letter-spacing:.07em;color:#6d28d9">RESULTADO DEL EJERCICIO</div>
            <div style="font-size:1.25rem;font-weight:850;color:#0f172a;margin:.4rem 0">Lₙ,w estimado</div>
            <div style="color:#526174;line-height:1.48">Número único obtenido desde una predicción. <b>No equivale a un valor medido conforme a ensayo.</b></div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.latex(
        ...
        r"\;\longrightarrow\;"
        r"\mathrm{procedimiento\ ISO\ 717\!-\!2}"
        r"\;\longrightarrow\;"
        r"L_{n,w,\mathrm{estimado}}"
    )

    st.warning(
        "**Alcance del ejercicio:** aplicaremos el procedimiento de ISO 717-2 a una curva predicha para aprender a construir "
        "el número único. En una aplicación normativa real, el espectro de entrada proviene de la medición definida por la "
        "norma de ensayo correspondiente."
    )

    # ------------------------------------------------------------------
    # 2 · FUENTE NORMALIZADA DE MEDICIÓN
    # ------------------------------------------------------------------
    st.markdown("## 2 · ¿Cómo se obtiene un espectro medido de ruido de impactos?")
    st.write(
        "Aquí aparece la **máquina de impactos normalizada**. No forma parte del modelo predictivo que usamos en el Laboratorio 1. "
        "Su función es generar una excitación mecánica reproducible cuando se realiza una medición de ruido de impactos en laboratorio "
        "o en un edificio."
    )
    ...
    if machine_img.exists():
        ...
        st.caption(
            "Render técnico didáctico · fuente normalizada de impactos con cinco martillos. "
            "La figura permite identificar la geometría y el mecanismo de excitación; no representa una marca comercial específica."
        )
    else:
        st.warning("Falta el asset `curso2_lab2_maquina_impactos_tecnica.png`.")

    st.markdown("### ¿Por qué no usamos una persona caminando?")
    st.write(
        "Porque una pisada humana cambia con la masa de la persona, el calzado, la velocidad y la forma de caminar. "
        "Para comparar resultados necesitamos una **fuente mecánica repetible**. La máquina controla la geometría y la secuencia "
        "de los impactos para que diferentes ensayos sean comparables."
    )

    st.markdown(
        """
        <div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin:.6rem 0 .85rem">
          <div style="padding:13px;border:1px solid #dbe4ee;border-radius:14px;background:#fff"><b>5 martillos</b><br><span style="color:#64748b">Actúan secuencialmente sobre el piso.</span></div>
          <div style="padding:13px;border:1px solid #dbe4ee;border-radius:14px;background:#fff"><b>≈ 500 g c/u</b><br><span style="color:#64748b">Masa definida de cada martillo.</span></div>
          <div style="padding:13px;border:1px solid #dbe4ee;border-radius:14px;background:#fff"><b>≈ 40 mm</b><br><span style="color:#64748b">Altura efectiva de caída.</span></div>
          <div style="padding:13px;border:1px solid #dbe4ee;border-radius:14px;background:#fff"><b>10 impactos/s</b><br><span style="color:#64748b">Cadencia global de la fuente.</span></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.info(
        "**No asignamos una fuerza pico fija a la máquina.** La fuerza instantánea depende del contacto martillo–piso y de la "
        "respuesta local del elemento. Lo que se normaliza es la fuente mecánica y su forma de excitación."
    )

    # ------------------------------------------------------------------
    # 3 · QUÉ MIDE EL MICRÓFONO
    # ------------------------------------------------------------------
    st.markdown("## 3 · La máquina excita el piso; el micrófono mide el resultado")
    st.write(
        "La máquina actúa sobre el piso del recinto emisor. En el recinto receptor, un micrófono registra el nivel de presión sonora "
        "producido por los impactos, **banda por banda**. Ese nivel directo todavía está influido por las características acústicas "
    ...
    st.markdown(
        ...
        <div style="border:1px solid #bfdbfe;background:#eff6ff;border-radius:15px;padding:14px 16px;margin:.5rem 0 1rem">
          <b>Secuencia física de la medición</b><br><br>
          <div style="font-size:1.03rem;font-weight:800;color:#1e3a8a;text-align:center">
            máquina normalizada → piso → vibración estructural → radiación al recinto receptor → micrófono
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.write(
        "Por eso el nivel que registra el micrófono no se interpreta todavía como Lₙ, L′ₙ o L′ₙT. "
        "Primero debemos aplicar la referencia correspondiente al contexto de medición."
    )

    # ------------------------------------------------------------------
    # 4 · LABORATORIO VS TERRENO
    # ------------------------------------------------------------------
    st.markdown("## 4 · La magnitud cambia según dónde y cómo se mide")
    st.write(
        "La misma fuente normalizada puede utilizarse para evaluar un elemento en condiciones controladas o una construcción real. "
        "Lo que cambia es **el contexto físico del ensayo y la forma de referenciar la influencia del recinto receptor**."
    )

    st.markdown(
        """
        <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin:.7rem 0 1rem">
          <div style="border:1px solid #bfdbfe;border-radius:18px;padding:18px;background:#eff6ff;min-height:258px">
            <div style="font-size:.72rem;font-weight:900;letter-spacing:.08em;color:#075985">LABORATORIO</div>
            <div style="font-size:1.35rem;font-weight:900;color:#0f172a;margin:.45rem 0">Lₙ(f)</div>
            <div style="color:#526174;line-height:1.5">
              Se caracteriza el <b>elemento de piso</b> bajo condiciones de ensayo controladas.<br><br>
              La influencia del recinto receptor se normaliza mediante el área de absorción equivalente.<br><br>
              <b>Número único:</b> Lₙ,w
            </div>
          </div>
          <div style="border:1px solid #a7f3d0;border-radius:18px;padding:18px;background:#ecfdf5;min-height:258px">
            <div style="font-size:.72rem;font-weight:900;letter-spacing:.08em;color:#047857">TERRENO · NORMALIZADO</div>
            <div style="font-size:1.35rem;font-weight:900;color:#0f172a;margin:.45rem 0">L′ₙ(f)</div>
            <div style="color:#526174;line-height:1.5">
              Se evalúa la <b>construcción real</b>, incluyendo encuentros, ejecución y caminos flanqueantes.<br><br>
              El resultado se normaliza mediante absorción equivalente.<br><br>
              <b>Número único:</b> L′ₙ,w
            </div>
          </div>
          <div style="border:1px solid #ddd6fe;border-radius:18px;padding:18px;background:#f5f3ff;min-height:258px">
            <div style="font-size:.72rem;font-weight:900;letter-spacing:.08em;color:#6d28d9">TERRENO · ESTANDARIZADO</div>
            <div style="font-size:1.35rem;font-weight:900;color:#0f172a;margin:.45rem 0">L′ₙT(f)</div>
            <div style="color:#526174;line-height:1.5">
              También corresponde al <b>edificio construido</b> y conserva los caminos reales de transmisión.<br><br>
              El resultado se estandariza mediante un tiempo de reverberación de referencia.<br><br>
              <b>Número único:</b> L′ₙT,w
            </div>
          </div>
        </div>
        """,
    ...
    st.markdown("### Laboratorio · normalización mediante absorción")
    st.latex(r"L_n=L_i+10\log_{10}\left(\frac{A}{A_0}\right)")
    st.latex(r"A_0=10\;\mathrm{m^2}")

    st.markdown("### Terreno · normalización mediante absorción")
    st.latex(r"L'_n=L'_i+10\log_{10}\left(\frac{A}{A_0}\right)")

    st.markdown("### Terreno · estandarización mediante reverberación")
    ...
    # 5 · QUÉ HACE LA w
    # ------------------------------------------------------------------
    st.markdown("## 5 · Recién después aparece la letra w")
    st.write(
        "Lₙ(f), L′ₙ(f) y L′ₙT(f) siguen siendo **curvas por frecuencia**. ISO 717-2 permite convertir cada una de esas curvas "
        "en una cantidad ponderada de número único."
    )
    ...
    st.markdown(
        """
        <div style="border:1px solid #dbe4ee;border-radius:16px;padding:15px 17px;background:#fff;margin:.5rem 0 .9rem">
          <b>w = weighted / ponderado</b><br>
          La letra w no significa promedio ni 500 Hz. Indica que la curva espectral fue transformada en una cantidad de número único mediante el procedimiento de ponderación correspondiente.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.success(
        "**Para nuestro ejercicio:** partiremos de Lₙ,final(f) predicho en el Laboratorio 1 y construiremos un **Lₙ,w estimado**. "
        "El procedimiento se practica igual, pero el origen del espectro sigue siendo una predicción."
    )
    ...
    # En esta actividad los símbolos son el contenido central de la respuesta.
    # Se aumenta su jerarquía visual para distinguir claramente prima, subíndices y T.
    st.markdown(
        """
        <style>
        div[role="radiogroup"] {
    ...
    answer_state=saved.get("stage1_context_answers",{})
    ...
    for item in questions:
        previous=answer_state.get(item["id"],{}) if isinstance(answer_state.get(item["id"]),dict) else {}
        previous_choice=previous.get("choice")
        idx=item["opts"].index(previous_choice) if previous_choice in item["opts"] else None
            ...
            if role=="Docente" and not projection_mode:
                ...
                if st.button("Comprobar y guardar",key=f"{class_id}_c2l2_s1_check_{item['id']}"):
                    if choice is None:
                        ...
                        answer_state[item["id"]]={
                            "choice":choice,
                            "correct":choice==item["correct"],
                            "updated_at":_now(),
                        }
                        saved["stage1_context_answers"]=answer_state
                        saved["updated_1"]=_now()
                        _save_future_state(_C2L2_CLASS_ID,saved)
                        st.rerun()

                result=answer_state.get(item["id"])
    ...
    if role!="Docente" or projection_mode:
        st.markdown(
            f"""
            <div style="border:1px solid #dbe4ee;border-radius:14px;padding:13px 16px;background:#f8fbff;margin:.6rem 0 .9rem">
              <b>Progreso de la actividad</b><br>
              Respuestas guardadas: <b>{answered}/3</b>
            </div>
            """,
            unsafe_allow_html=True,
        )
