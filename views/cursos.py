...
def _c2l2_curve_control(saved,prefix,curve,require_limit=False,unlock_reading=False,auto_limit_feedback=False):
    ...
    # Reinicio robusto: el botón NO modifica directamente la key del slider.
    # Solo marca una solicitud. En el rerun siguiente, antes de instanciar el
    # slider, aplicamos el reinicio de forma segura.
    reset_request_key=f"{prefix}_reset_requested"
    if st.session_state.get(reset_request_key):
        st.session_state[position_key]=60
        st.session_state[position_ok_key]=False
        st.session_state[reset_request_key]=False

    def _request_curve_reset():
...
def _c3l1_stage2_impl(lab: dict, saved: dict, deps: Dict[str, Any]):
    ...
    st.write(
        'Explora físicamente la cadena de medición. Al seleccionar un componente cambia el render, '
        'su función y el punto de la cadena donde actúa.'
    )

    components_map = {
        'Micrófono': (
            'Presión acústica → señal eléctrica',
            'Convierte las variaciones de presión acústica que llegan a su membrana en una señal eléctrica proporcional.',
            'No interpreta molestia ni normativa: transforma el fenómeno físico.',
            '#0f86b8',
        ),
        'Preamplificador': (
            'Acondicionamiento de señal',
            'Recibe la señal eléctrica de muy bajo nivel proveniente del micrófono y la adapta para el procesamiento posterior.',
            'Debe preservar la información útil sin introducir una alteración significativa.',
            '#7c3aed',
        ),
        'Filtros / ponderación': (
            'Respuesta en frecuencia',
            'Aplica la ponderación frecuencial seleccionada, por ejemplo A, C o Z.',
            'Ponderación frecuencial no es lo mismo que respuesta temporal.',
            '#db2777',
        ),
        'Procesador RMS / integrador': (
            'Nivel eficaz e integración',
            'Procesa la señal temporal y permite obtener niveles eficaces e integrar energía durante un intervalo.',
            'Aquí se construyen descriptores que desarrollarás en las etapas siguientes.',
            '#d97706',
        ),
        'Memoria': (
            'Registro de datos',
            'Almacena niveles, historia temporal, configuraciones y metadatos necesarios para reconstruir la medición.',
            'Un número sin configuración, tiempo y contexto puede ser insuficiente para interpretar una campaña.',
            '#16a34a',
        ),
        'Calibrador': (
            'Comprobación de la cadena',
            'Se acopla al micrófono y aplica una señal acústica de referencia para comprobar la respuesta del sistema.',
            'La verificación de campo no reemplaza la calibración metrológica periódica.',
            '#dc2626',
        ),
    }

    part = st.segmented_control(
        'Explora los bloques del instrumento',
        list(components_map),
        default='Micrófono',
        key='c3_s2_part',
    )
    ...
    render_assets = {
        ...
        'Filtros / ponderación': 'curso3_sonometro_filtros_hires.webp',
    ...
    st.markdown(
        ...
        <div class="c3-kicker">{part}</div>
        <b>{title}</b>
        <p>{description}</p>
        <div style="color:#52687d">{idea}</div>
        </div>''',
        unsafe_allow_html=True,
    )
...
def _c3l1_stage3_impl(lab: dict, saved: dict, deps: Dict[str, Any]):
    ...
    st.markdown(
        ...
        <div class="c3-grid">
          ...
          <div class="c3-card orange"><div class="c3-kicker">EVENTO</div><b>Cambio delimitado</b>
          <p>Aparece, crece, alcanza un máximo y desaparece. Su duración importa tanto como su máximo.</p></div>
    ...
    st.markdown('## 2. Explora tres ambientes acústicos')
    ...
    t = np.arange(0, 180, 1, dtype=float)

    if scenario == 'Tránsito urbano':
        y = 58 + 1.4*np.sin(t/7) + rng.normal(0,0.8,len(t))
        for c,a,w in [(24,8,3),(55,5,5),(91,10,3),(133,7,4),(160,9,3)]:
            y += a*np.exp(-.5*((t-c)/w)**2)
        scenario_note = 'Fondo vial + pasos de vehículos con distinta intensidad.'
    elif scenario == 'Obra con eventos':
        y = 55 + rng.normal(0,0.9,len(t))
        for c,a,w in [(18,14,1.5),(37,11,2),(65,16,1.3),(101,13,2),(128,17,1.5),(154,12,2)]:
            y += a*np.exp(-.5*((t-c)/w)**2)
        scenario_note = 'Fondo moderado + eventos impulsivos/intermitentes de obra.'
    else:
        y = 61 + 0.7*np.sin(t/11) + 0.35*np.sin(t/2.8) + rng.normal(0,0.25,len(t))
        y += 1.3*((t>70)&(t<125))
        scenario_note = 'Fuente técnica estable con un ciclo de funcionamiento ligeramente más alto.'

    vals = {
        'LAeq': _c3l1_laeq(y),
        'Lmax': float(np.max(y)),
        'Lmin': float(np.min(y)),
        'L10': _c3l1_exceedance_percentile(y,10),
        'L50': _c3l1_exceedance_percentile(y,50),
        'L90': _c3l1_exceedance_percentile(y,90),
    }
    ...
    ax.grid(alpha=.2)
    styles={'LAeq':'--','Lmax':':','Lmin':':','L10':'-.','L50':'-.','L90':'-.'}
    for k in show:
        ax.axhline(vals[k],ls=styles[k],lw=1.6,label=f'{k} = {vals[k]:.1f}')
    if show:
    ...
    c2.metric('Lmax / Lmin',f'{vals["Lmax"]:.1f} / {vals["Lmin"]:.1f} dB(A)')
    ...
    q3=[
        ...
        ('Una diferencia L10−L90 grande sugiere:',['Ambiente más variable','Ambiente necesariamente silencioso'],'Ambiente más variable',
         'Una separación grande entre la zona alta y la zona baja de excedencia indica mayor variabilidad temporal.'),
        ('LAeq representa:',['Energía acústica equivalente durante T','El máximo del periodo'],'Energía acústica equivalente durante T',
         'LAeq resume energéticamente todo el periodo de integración.'),
    ]
    ans={}
    score=0
