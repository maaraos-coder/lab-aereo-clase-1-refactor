...
# apariencia al cambiar entre Mis clases, Evaluaciones y Laboratorios.
# ---------------------------------------------------------------------
st.markdown(
    """
    <style>
    /* ====== RADIO CARDS GLOBALES DEL SIDEBAR ====== */
    ...
    /* Fallback para algunas versiones de Streamlit */
    section[data-testid="stSidebar"] div[data-testid="stRadio"] label[aria-checked="true"],
    section[data-testid="stSidebar"] div[data-testid="stRadio"] [data-checked="true"] {
        background:linear-gradient(
            135deg,
            rgba(4,104,154,.96),
            rgba(13,140,179,.76)
    ...
    /* ====== PANEL VISUAL PARA BLOQUES DE NAVEGACIÓN ======
       Si el radio está dentro de un contenedor con borde, ese contenedor
       conserva la misma estética en cualquier vista. */
    section[data-testid="stSidebar"] div[data-testid="stVerticalBlockBorderWrapper"] {
        width:100% !important;
        max-width:100% !important;
        box-sizing:border-box !important;
        border:1px solid rgba(89,212,239,.30) !important;
    ...
    /* Evitar estilos primary rojos heredados en radios/navegación */
    section[data-testid="stSidebar"] button[kind="primary"] {
        --primary-color:#0c8caf !important;
    }
    </style>
    """,
    unsafe_allow_html=True,
...
# Override final: fuerza ancho y alto idénticos incluso en wrappers BaseWeb.
st.markdown(
    r"""
    <style>
    /* =========================================================
...
with st.sidebar:
    ...
    # Ruta de aprendizaje: mismo lenguaje visual que la navegación principal.
    # Se aplica a Alumno y Docente porque ambos usan el mismo selector de etapas.
    st.markdown(
        """
        <style>
        section[data-testid="stSidebar"] div[class*="st-key-selected_stage_lab_"] [role="radiogroup"] {
            gap: .34rem !important;
        }
        section[data-testid="stSidebar"] div[class*="st-key-selected_stage_lab_"] [role="radiogroup"] > label {
            width: 100% !important;
            margin: 0 !important;
        ...
        section[data-testid="stSidebar"] div[class*="st-key-selected_stage_lab_"] [role="radiogroup"] > label:hover {
            background: rgba(21,111,160,.30) !important;
            border-color: rgba(89,212,239,.48) !important;
        }
        section[data-testid="stSidebar"] div[class*="st-key-selected_stage_lab_"] [role="radiogroup"] > label:active {
            transform: translateY(1px);
        }
        section[data-testid="stSidebar"] div[class*="st-key-selected_stage_lab_"] [role="radiogroup"] > label:has(input:checked) {
        ...
        section[data-testid="stSidebar"] div[class*="st-key-selected_stage_lab_"] [data-baseweb="radio"] {
            display:flex !important;
            opacity:1 !important;
            visibility:visible !important;
            flex:0 0 auto !important;
        ...
        section[data-testid="stSidebar"] div[class*="st-key-selected_stage_lab_"] input[type="radio"] {
            accent-color:#59d4ef !important;
        }
        section[data-testid="stSidebar"] div[class*="st-key-selected_stage_lab_"] [data-testid="stMarkdownContainer"] p {
            font-size:.75rem !important;
